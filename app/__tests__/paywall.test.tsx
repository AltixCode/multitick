...
describe("Paywall", () => {
  ...
  // This paywall is a swipe-paged carousel now, not a static numbered list —
  // one dot per benefit, so the count of dots is a real assertion on the new
  // structure rather than just re-checking copy through `t()`.
  it("shows one paging dot per benefit", async () => {
    const { getByLabelText } = await renderWithProviders(<Paywall />);
    expect(getByLabelText("1 / 4")).toBeTruthy();
    expect(getByLabelText("4 / 4")).toBeTruthy();
  });
